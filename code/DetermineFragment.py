...
def get_SV_orient(bp_result, dict_matrix, res, merge):
    ...
    for k in bp_result.index:
        ...
        cut_off *= 2
        # calculate orient
        cut_1 = cut_2 = merge // res
        # sns.heatmap(cur_matrix, cmap='Reds')
        # plt.show()
        cut_matrix = {}
        cut_std = []
...
def plot_link(SV_fragment, chrom, start, end, chrom_list, plot_start, plot_end, savepath):
    color_list = []
    ...
    for i in range(len(chrom_list)):
        ...
        ax.add_patch(patches.Rectangle((chrom_start, 2), width=chrom_length, height=1, color='gray', zorder=1, alpha=.4, linewidth=0))
        plt.text(chrom_start, 6, chrom_list[i], fontsize=16, weight='bold')
        chrom_start = chrom_start + chrom_length + 2

    ## plot fragment
    for i in range(len(start)):
        cur_start = dict_start[chrom[i]]
        textPos = cur_start + (start[i]-plot_start[chrom[i]]) + (end[i] - start[i]) // 2
        plt.text(textPos, 4, chr(65 + i), fontsize=10)
        ax.add_patch(patches.Rectangle((cur_start + (start[i]-plot_start[chrom[i]]), 2), width=end[i]-start[i], height=1, color='black', zorder=1, alpha=.4, linewidth=0))
    ...
    for i in SV_fragment.index:
        ...
        ax.add_patch(patches.Rectangle((start1, h), width=end1-start1, height=1, facecolor=mcolors.TABLEAU_COLORS[colors[i % len(colors)]], zorder=1, linewidth=2))
        ax.add_patch(patches.Rectangle((start2, h), width=end2-start2, height=1, facecolor=mcolors.TABLEAU_COLORS[colors[i % len(colors)]], zorder=1, linewidth=2))
    ...
    ylim = 0 - SV_fragment.shape[0] * 5 - 5
    plt.xlim([0, chrom_start])
    plt.ylim([ylim, 8])
    plt.axis('off')
    fig.subplots_adjust(0.1, 0.01, 0.9, 0.95)
    plt.savefig(savepath + '/result/fragment_link.png')
    # plt.show()


def constructPath(SV_fragment, chrom, start, end):
    all_path = []
    all_orient = []
    SV_fragment['flag'] = 0
    ...
    for i in SV_fragment.index:
        ...
        for j in range(len(start)):
            if start1 == start[j] and end1 == end[j] and chrom[j] == row['chrom1']:
                SV_fragment.loc[i, 'fragment1'] = chr(j+65)
        start2 = row['start2']
        end2 = row['end2']
        for j in range(len(start)):
            if start2 == start[j] and end2 == end[j] and chrom[j] == row['chrom2']:
                SV_fragment.loc[i, 'fragment2'] = chr(j+65)
...
def plot_SV_genome(path, orient, chrom, start, end, savepath):
    fig = plt.figure(figsize=(10, 4))
    ax = fig.add_subplot(111)
    h = 0
    length_list = []
    ...
    for i in range(len(path)):
        ...
        for j in range(len(path[i])):
            index = ord(path[i][j])-65
            length = end[index] - start[index] + 1
            count += length
            textPos = cur_start + length // 2
            plt.text(textPos, h+2, path[i][j], fontsize=22, weight='bold')
            if orient[i][j] == '+':
                pos1 = cur_start
                pos2 = pos1 + length
            else:
            ...
            ax.annotate("",
                        ...
                        xytext=[pos1, h],
                        size=10,
                        arrowprops=dict(color=mcolors.TABLEAU_COLORS[colors[i % len(colors)]], headwidth=25, width=10, headlength=10), zorder=0)
    ...
    plt.ylim([h, 3])
    plt.axis('off')
    plt.savefig(savepath + '/result/CGR_event.png', bbox_inches='tight', pad_inches=0)
    # plt.show()
...
def matrix_plot(matrix, interval, chrom_text_pos, chrom_text, savepath):
    ...
    for i in range(len(interval)):
        j = i*2
        plt.text(chrom_text_pos[j]-10, h, chrom_text[j], fontsize=16, weight='bold')
        plt.text(chrom_text_pos[j+1]-10, h, chrom_text[j+1], fontsize=16, weight='bold')
        h -= 15

    ax.axis('off')

    fig.subplots_adjust(0.1, 0.05, 0.9, 0.9)

    plt.savefig(savepath + '/tmp/page1_hicmap.png')
    # plt.show()
...
def FragmentAssembly(SV_file, resolution, dict_matrix, chrom_list, dict_count, savepath):
    ...
    chrom, start, end = genomeFragment(SV_fragment)
    # print(SV_fragment, chrom, start, end)
    # plot fragment
    dict_start = {}
    dict_end = {}
    for i in range(len(chrom_list)):
    ...
    plot_link(SV_fragment, chrom, start, end, chrom_list, dict_start, dict_end, savepath)
    # plot Hi-C matrix
    chrom_matrix, interval, chrom_text_pos, chrom_text = combine_matrix(dict_matrix, dict_start, dict_end, chrom_list, resolution)
    matrix_plot(chrom_matrix, interval, chrom_text_pos, chrom_text, savepath)
    # assembly
    all_path, all_orient = constructPath(SV_fragment, chrom, start, end)
    # plot assembly
    plot_SV_genome(all_path, all_orient, chrom, start, end, savepath)
    return all_path, all_orient, chrom, start, end
