...
def plot_link(SV_fragment, chrom, start, end, chrom_list, plot_start, plot_end, savepath):
    ...
    for i in range(len(chrom_list)):
        ...
        plt.text(chrom_start, 7, chrom_list[i], fontsize=10, weight='bold')
    ...
    for i in range(len(start)):
        ...
        plt.text(textPos, 3, chr(65 + i), fontsize=10)
...
def matrix_plot(matrix, interval, chrom_text_pos, chrom_text, savepath):
    ...
    for i in range(len(interval)):
        if i > 0:
            h -= 15
        j = i*2
        plt.text(chrom_text_pos[j]-10, h, chrom_text[j], fontsize=10, weight='bold')
        plt.text(chrom_text_pos[j+1]-10, h, chrom_text[j+1], fontsize=10, weight='bold')
    ax.axis('off')
...
def FragmentAssembly(SV_file, resolution, dict_matrix, chrom_list, dict_count, savepath):
    ...
    canvas_link = plot_link(SV_fragment, chrom, start, end, chrom_list, dict_start, dict_end, savepath)
    # plot Hi-C matrix
    chrom_matrix, interval, chrom_text_pos, chrom_text = combine_matrix(dict_matrix, dict_start, dict_end, chrom_list, resolution)
    canvas_matrix = matrix_plot(chrom_matrix, interval, chrom_text_pos, chrom_text, savepath)
    # assembly
    all_path, all_orient = constructPath(SV_fragment, chrom, start, end)
    # plot assembly
    canvas_assembly = plot_SV_genome(all_path, all_orient, chrom, start, end, savepath)
    return all_path, all_orient, chrom, start, end, canvas_matrix, canvas_link, canvas_assembly
