...
def main():
    ...
    tmp_path = os.path.dirname(os.path.dirname(fragsFile))
    print(tmp_path)
    ...
    if resolution:
        ...
        for ch in allFragsDic:
            if (len(allFragsDic[ch]) == 0):
                ...
                print("ERROR - the chromosome " + ch + " has " + str(len(allFragsDic[ch])) + " valid fragments/bins and should be removed from the input fragment information !!! ")                
            else:
...
def fit_Spline(mainDic,x,y,yerr,infilename,outfilename,biasDic,outliersline,outliersdist,observedIntraInRangeSum, possibleIntraInRangeCount, possibleInterAllCount, observedInterAllCount, observedIntraAllSum, observedInterAllSum, biasLowerBound, biasUpperBound, resolution, passNo):
    ...
    if not interOnly:
        ...
        print('*'*20)
        print(tmp_path +'/result/dis2cnt')
        f=open(tmp_path + '/result/dis2cnt','w')
        former=inf
        for i in range(len(splineX)):
            if newSplineY[i]>=former:
                # f.write('long\t'+str(newSplineY[i]*observedIntraInRangeSum)+'\n')
                break
            former=newSplineY[i]         
            f.write(str(splineX[i])+'\t'+str(newSplineY[i]*observedIntraInRangeSum)+'\n')
        f.write('-1\t'+str(observedInterAllSum * interChrProb)+'\n')
        f.close()            
    return
