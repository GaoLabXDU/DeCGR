...
'''
def run():
    result_file = "assembly_result.txt"
    res = 25000
    hic_file = "/mnt/d/detectTAD/Data/normalization/4DNFI18UHVRO.mcool"
    path, orient, chrom, start, end = read_one_assembly(result_file, res)
    # ICE or sweight
    norm = "sweight"
    hic_file = hic_file + '::/resolutions/' + str(res)
        ...
        for i in range(len(chrom_list)):
            for j in range(i, len(chrom_list)):
                ...
                if i == j:
                    ...
                    dict_norm[name] = normed
                else:
                    dict_norm[name] = cur_matrix
        # glob_exp = global_expect(dict_norm, chrom_list)
        # SV_matrix = joint_SV(path, orient, chrom, start, end, dict_matrix)

        # matrix, interval = adjust_matrix(SV_matrix, glob_exp, path, start, end)
        # plot_assembly_matrix(SV_matrix, interval, path, orient, path)
    elif norm == "sweight":
        # running correct-CNV
        genome = "hg38"
        enzyme = "MboI"
        correct.run(hic_file, genome, enzyme, res)
        # load matrix
        for i in range(len(chrom_list)):
            for j in range(i, len(chrom_list)):
                name = str(chrom_list[i]) + '_' + str(chrom_list[j])
