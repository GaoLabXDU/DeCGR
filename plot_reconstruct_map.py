...
def plot_assembly_matrix(matrix, interval, path, orient, savepath):
    ...
    for i in range(len(interval)):
        if interval[i] != 0:
            ax.plot([x[(n - interval[i]), 0], x[(n-interval[i]), interval[i]]], [y[(n - interval[i]), 0], 0],
                    color="black", linestyle='--', linewidth=2)
            ax.plot([x[(n-interval[i]), n], x[(n - interval[i]), interval[i]]], [y[(n-interval[i]), n], 0],
                    color="black", linestyle='--', linewidth=2)
    ...
    for i in range(len(interval)-1):
        ...
        plt.text(textPos, h+2, path[i],  fontdict={'family': 'Arial', 'size': 16, 'color': 'black', 'weight':'bold'})
        ax.annotate("",
                xy=[pos1, h],
                xytext=[pos2, h],
                size=10,
                arrowprops=dict(color='gray', headwidth=20, width=10, headlength=10,  ec="black", lw=1), zorder=0)

    ax.axis('off')
    plt.ylim([-8, n])
    plt.subplots_adjust(0.1, 0.1, 0.9, 0.8)
    plt.savefig(savepath + '/result/restruct_map.png')
    # plt.show()
